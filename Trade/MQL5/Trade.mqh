...
/*
aia cu string la buy si sell sia astea sa o modific si sa introduc si _symbol daca cumva e NULL
*/
...
public:
   ...
   bool              PositionClosePartial(const ulong ticket, const double volume, const ulong deviation = ULONG_MAX, string comment = NULL);
...
//| Partial close specified opened position (for hedging mode only)  |
//+------------------------------------------------------------------+
bool CTrade::PositionClosePartial(const ulong ticket, const double volume, const ulong deviation, string comment = NULL)
  {
//--- check stopped
   if(IsStopped(__FUNCTION__))
      return(false);
